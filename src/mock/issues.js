...
export function IssueRoutes() {
  this.get("/issues", (schema, request) => {
    ...
    const filters = _.pick(params, ["issueType", "severity"]);
    const filtered =
    ...
    const filteredStatus =
      params.status !== undefined
        ? filtered.filter((issue) => issue.status === params.status)
        : filtered;

    let filteredOrigin =
      originIds.length > 0
        ? filteredStatus.filter((issue) => originIds.includes(issue.originId))
        : filteredStatus;
